...
def run(rank, world_size, dataset):
    try:
        ...
        if rank == 0:
            # Save test dataset to file for visualization in notebook
            test_data_list = [test_dataset[i] for i in range(len(test_dataset))]

            data_dir = osp.join(osp.dirname(osp.abspath(__file__)), '..', 'data')

            save_path = osp.join(data_dir, 'test_dataset.pt')
            torch.save(test_data_list, save_path)
        ...
        if rank == 0:
            ...
            model_path = osp.join(root_path, 'model.pth')
...
if __name__ == '__main__':
    root_path = osp.join(osp.dirname(osp.abspath(__file__)), '..')
    ...
    try:
        mp.spawn(run, args=(world_size, dataset), nprocs=world_size)  # type: ignore
