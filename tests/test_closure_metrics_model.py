...
def _minimal_train_batch(ws: int = 3, *, device=torch.device('cpu')) -> _MinimalTrainBatch:
    ...
    d = Data(
        ...
        c3_prev_local=torch.zeros(ws, 3, dtype=torch.float32, device=device),
        c4_prev_local=torch.zeros(ws, 3, dtype=torch.float32, device=device),
        prev_bridge_ref_valid=torch.zeros(ws, dtype=torch.bool, device=device),
